...
describe('Twitch service', () => {
  ...
  describe('refreshTwitchToken', () => {
    ...
    it('should throw error if client ID is missing', async () => {
      ...
      await expect(
        twitch.refreshTwitchToken('refresh-token', mockSecrets)
      ).rejects.toThrow('Server configuration error for Twitch token refresh.');
    });
    ...
    it('should handle Twitch API errors', async () => {
      ...
      await expect(
        twitch.refreshTwitchToken('invalid-refresh-token', mockSecrets)
      ).rejects.toThrow('Failed to refresh Twitch token');
    });
  });
  ...
  describe('validateTwitchToken', () => {
    ...
    it('should throw error for invalid token', async () => {
      ...
      await expect(
        twitch.validateTwitchToken('invalid-token')
  ...
  describe('getAppAccessToken', () => {
    ...
    it('should throw error if no access token in response', async () => {
      ...
      await expect(
        twitch.getAppAccessToken(mockSecrets)
      ).rejects.toThrow('Failed to get app access token');
    });
  ...
  describe('addModerator', () => {
    ...
    it('should successfully add a moderator (204 response)', async () => {
      ...
      const result = await twitch.addModerator(
        'broadcasterlogin',
        'broadcaster-123',
        'moderator-456',
        mockSecrets
