...
router.post("/auth", async (req, res) => {
    const log = logger_1.logger.child({ endpoint: "/api/viewer/auth" });
    try {
...
router.get("/preferences/:channel", auth_1.authenticateApiRequest, async (req, res) => {
    ...
    const log = logger_1.logger.child({ endpoint: "/api/viewer/preferences/:channel", channel, username });
    try {
        if (!channel) {
        ...
        let globalPrefs = {};
        try {
            const userDoc = await firestore_1.db.collection("ttsUserPreferences").doc(username).get();
            if (userDoc.exists) {
        ...
        try {
            const musicDoc = await firestore_1.db.collection(firestore_1.COLLECTIONS.MUSIC_SETTINGS).doc(channel).get();
            if (musicDoc.exists) {
...
router.put("/preferences/:channel", auth_1.authenticateApiRequest, async (req, res) => {
    const { channel } = req.params;
    if (!req.user) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    ...
    try {
        ...
        if (updates.pitch !== undefined) {
            if (updates.pitch === null || (0, utils_1.validatePitch)(updates.pitch)) {
        ...
        if (updates.speed !== undefined) {
            if (updates.speed === null || (0, utils_1.validateSpeed)(updates.speed)) {
        ...
        if (updates.emotion !== undefined) {
            const normalized = updates.emotion === null ? null : (0, utils_1.normalizeEmotion)(updates.emotion);
            if (normalized === null || (0, utils_1.validateEmotion)(normalized)) {
        ...
        if (updates.language !== undefined) {
            if (updates.language === null || (0, utils_1.validateLanguageBoost)(updates.language)) {
        ...
        await firestore_1.db.collection("ttsUserPreferences").doc(username).set(updateData, { merge: true });
...
router.get("/preferences", auth_1.authenticateApiRequest, async (req, res) => {
    if (!req.user) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const username = req.user.userLogin;
    const log = logger_1.logger.child({ endpoint: "/api/viewer/preferences", username });
    try {
        // Load global user preferences
        let globalPrefs = {};
        try {
...
router.put("/preferences", auth_1.authenticateApiRequest, async (req, res) => {
    if (!req.user) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
...
router.post("/ignore/tts/:channel", auth_1.authenticateApiRequest, async (req, res) => {
    const { channel } = req.params;
    if (!req.user) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    ...
    const username = req.user.userLogin;
    const log = logger_1.logger.child({ endpoint: "/api/viewer/ignore/tts/:channel", channel, username });
    try {
        if (!channel) {
            res.status(400).json({ error: "Channel is required" });
            return;
        }
        const channelDocRef = firestore_1.db.collection(firestore_1.COLLECTIONS.TTS_CHANNEL_CONFIGS).doc(channel);
...
router.post("/ignore/music/:channel", auth_1.authenticateApiRequest, async (req, res) => {
    const { channel } = req.params;
    if (!req.user) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    ...
    const username = req.user.userLogin;
    const log = logger_1.logger.child({ endpoint: "/api/viewer/ignore/music/:channel", channel, username });
    try {
        if (!channel) {
            res.status(400).json({ error: "Channel is required" });
            return;
        }
        const musicDocRef = firestore_1.db.collection(firestore_1.COLLECTIONS.MUSIC_SETTINGS).doc(channel);
