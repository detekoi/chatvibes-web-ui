...
exports.TWITCH_HELIX_BASE = exports.TWITCH_VALIDATE_URL = exports.TWITCH_TOKEN_URL = void 0;
exports.refreshTwitchToken = refreshTwitchToken;
exports.getValidTwitchTokenForUser = getValidTwitchTokenForUser;
exports.validateTwitchToken = validateTwitchToken;
exports.makeTwitchApiRequest = makeTwitchApiRequest;
...
exports.TWITCH_TOKEN_URL = TWITCH_TOKEN_URL;
const TWITCH_VALIDATE_URL = "https://id.twitch.tv/oauth2/validate";
exports.TWITCH_VALIDATE_URL = TWITCH_VALIDATE_URL;
const TWITCH_HELIX_BASE = "https://api.twitch.tv/helix";
exports.TWITCH_HELIX_BASE = TWITCH_HELIX_BASE;
/**
...
async function refreshTwitchToken(currentRefreshToken, secrets) {
    if (!secrets.TWITCH_CLIENT_ID || !secrets.TWITCH_CLIENT_SECRET) {
        logger_1.logger.error("Twitch client ID or secret not configured for token refresh.");
        throw new Error("Server configuration error for Twitch token refresh.");
    }
    logger_1.logger.info("Attempting to refresh Twitch token...");
    try {
        const refreshResponse = await axios_1.default.post(TWITCH_TOKEN_URL, null, {
            params: {
        ...
        if (!newAccessToken || !newRefreshToken) {
            logger_1.logger.error({ responseData: (0, logger_1.redactSensitive)(refreshResponse.data) }, "Missing access_token or refresh_token in refresh response");
            throw new Error("Twitch did not return the expected refreshed tokens.");
        }
        const expiresAt = new Date(Date.now() + (expiresIn * 1000));
        logger_1.logger.info({ expiresAt: expiresAt.toISOString() }, "Successfully refreshed Twitch token");
        return {
            newAccessToken,
            newRefreshToken,
            expiresAt,
...
async function getValidTwitchTokenForUser(userLogin, secrets) {
    const log = logger_1.logger.child({ userLogin });
    if (!firestore_1.db || !config_1.secretManagerClient) {
        log.error("Firestore or Secret Manager client not initialized!");
        throw new Error("Server configuration error.");
    }
    const userDocRef = firestore_1.db.collection(firestore_1.COLLECTIONS.MANAGED_CHANNELS).doc(userLogin);
    ...
    if (expiresAt && (expiresAt.getTime() - now.getTime()) > bufferTime) {
        ...
        try {
            const secretName = `projects/${config_1.config.GCLOUD_PROJECT}/secrets/twitch-access-token-${twitchUserId}`;
    ...
    try {
        ...
        const refreshSecretName = `projects/${config_1.config.GCLOUD_PROJECT}/secrets/twitch-refresh-token-${twitchUserId}`;
        const [refreshVersion] = await config_1.secretManagerClient.accessSecretVersion({
        ...
        await Promise.all([
            config_1.secretManagerClient.addSecretVersion({
                parent: `projects/${config_1.config.GCLOUD_PROJECT}/secrets/twitch-access-token-${twitchUserId}`,
                payload: { data: Buffer.from(newAccessToken) },
            }),
            config_1.secretManagerClient.addSecretVersion({
                parent: `projects/${config_1.config.GCLOUD_PROJECT}/secrets/twitch-refresh-token-${twitchUserId}`,
...
async function validateTwitchToken(accessToken) {
    try {
        const response = await axios_1.default.get(TWITCH_VALIDATE_URL, {
...
async function getAppAccessToken(secrets) {
    try {
        const response = await axios_1.default.post(TWITCH_TOKEN_URL, null, {
            params: {
                client_id: secrets.TWITCH_CLIENT_ID,
                client_secret: secrets.TWITCH_CLIENT_SECRET,
...
async function getUserIdFromUsername(username, secrets) {
    try {
        ...
        const response = await axios_1.default.get(`${TWITCH_HELIX_BASE}/users`, {
...
async function addModerator(broadcasterLogin, broadcasterId, moderatorUserId, secrets) {
    try {
        ...
        const response = await axios_1.default.post(`${TWITCH_HELIX_BASE}/moderation/moderators`, null, {
            params: {
        ...
        if (response.status === 204) {
            logger_1.logger.info({ broadcasterLogin, moderatorUserId }, "[addModerator] Successfully added moderator");
            return { success: true };
        }
        ...
        const log = logger_1.logger.child({ broadcasterLogin, moderatorUserId, status });
        ...
        log.error({
            errorMessage,
            errorData: (0, logger_1.redactSensitive)(errorData),
