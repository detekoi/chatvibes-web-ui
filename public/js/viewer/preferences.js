 ...
 */
export function initPreferencesModule(context, services, deps = {}) {
    ...
    function attachPreviewHandlers() {
        ...
        [voiceSelect, pitchSlider, speedSlider, emotionSelect, languageSelect, englishNormalizationCheckbox]
            .filter(Boolean)
            .forEach(el => el.addEventListener('change', markSettingsAsDirty));
        ...
        async function testVoice() {
            ...
            const payload = {
                ...
                pitch: pitchHasOverride ? Number(pitchSlider?.value ?? 0) : null,
                speed: speedHasOverride ? Number(speedSlider?.value ?? 1) : null,
            ...
            const buttons = [previewBtn, previewBtnMobile].filter(Boolean);
    ...
    function updateHints(keys) {
        ...
        toUpdate.forEach(key => {
            ...
            el.style.display = '';
            el.offsetHeight; // force reflow for immediate update
        });
    }
