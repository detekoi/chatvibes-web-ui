...
document.addEventListener("DOMContentLoaded", () => {
  (async function bootstrap() {
    ...
    async function initializeAuth(tokenParam) {
      ...
      const sessionTokenParam = urlParams2.get("session_token");
      const validatedParam = urlParams2.get("validated");
      const errorParam = urlParams2.get("error");
      const messageParam = urlParams2.get("message");
      if (errorParam) {
    ...
    async function requireTwitchAuth(tokenParam) {
      ...
      loginButton.onclick = async () => {
        try {
          ...
          const authUrl = tokenParam && state.currentChannel ? `${apiBaseUrl}/auth/twitch/viewer?token=${encodeURIComponent(tokenParam)}&channel=${encodeURIComponent(state.currentChannel)}` : `${apiBaseUrl}/auth/twitch/viewer`;
