 ...
 * @param {string} context.botApiBaseUrl
 ...
 * @param {{displayIgnoreList: (type: 'tts'|'music', users: string[]) => void}} dependencies
 * @returns {{initialize: () => Promise<void>, loadSettings: () => Promise<void>}}
 */
export function initSettingsModule(context, services, dependencies) {
    ...
    function attachVoicePreview() {
        ...
        const testVoice = async (isMobile = false) => {
            ...
            const testButton = isMobile ? voiceTestBtnMobile : voiceTestBtn;
            ...
            const buttons = [voiceTestBtn, voiceTestBtnMobile].filter(Boolean);
