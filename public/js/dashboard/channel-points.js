...
function initChannelPointsModule(context, services, deps = {}) {
  ...
  async function loadChannelPointsConfig() {
    ...
    try {
      ...
      if (cpStatusLine) {
        ...
        cpStatusLine.textContent = `${rewardInfo} \xB7 Last synced: ${lastSynced}`;
      }
    } catch (error) {
  ...
  async function saveChannelPointsConfig(isAuto = false) {
    if (testMode) {
      ...
      if (cpStatusLine) cpStatusLine.textContent = "No reward created \xB7 Last synced: " + (/* @__PURE__ */ new Date()).toLocaleTimeString();
      return;
    }
    if (!getSessionToken()) {
    ...
    const payload = {
      ...
      contentPolicy: {
        ...
        bannedWords: (cpBannedWords?.value || "").split(",").map((word) => word.trim()).filter(Boolean)
      }
    };
    if (!isAuto) {
    ...
    try {
      ...
      if (cpStatusLine) {
        ...
        cpStatusLine.textContent = `${idPart} \xB7 Last synced: ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}`;
      }
  ...
  async function testChannelPointsRedeem() {
    if (testMode) {
      const text2 = prompt("Enter a test message to simulate a redemption:");
      if (text2) showToast("Test validated \u2713 (test mode)", "success");
      return;
    }
    if (!getSessionToken()) {
      showToast("Authentication required", "error");
...
export {
  initChannelPointsModule
};
//# sourceMappingURL=channel-points.js.map
